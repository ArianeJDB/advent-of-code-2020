...
function resolve(input) {
    let result = 0;
    input.map(str => {
        const minNum= /\d{1,}/.exec(str)[0]
        const maxNum = /\d{1,}-(\d{1,})/.exec(str)[1]
        const letter = /\d{1,}-\d{1,}\s(\D)/.exec(str)[1]
        const newString = /\d{1,}-\d{1,}\s\D:\s(\D{1,})/.exec(str)[1]
        
        const f = newString.split('').filter(char => char === letter)
        if(f.length >= minNum && f.length <= maxNum) {
            result += 1
        }
    })
    return result
