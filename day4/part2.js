...
function resolve(input) {
    // ha sido la primera solución. TODO sacar cada validación a una función aparte
    let arr = []
    let result = 0;
    for (let i = 0; i < input.length; i++) {
        
        const passportArray = input[i].split(/\s+/)
        let obj = {}
        for (let j = 0; j < passportArray.length; j++) {
            obj[passportArray[j].split(':')[0]] = passportArray[j].split(':')[1]
        }
        arr.push(obj)
    }
    ...
    newArr.map(passport => {
        ...
        const hgtRegex = /^(\d{1,})(\D{2})$/
        if(passport.hgt && hgtRegex.exec(passport.hgt)) {
            if(hgtRegex.exec(passport.hgt)[2] === "cm" && hgtRegex.exec(passport.hgt)[1] >= 150 && hgtRegex.exec(passport.hgt)[1] <= 193) {
                hgt = true
            } else if(hgtRegex.exec(passport.hgt)[2] === "in" && hgtRegex.exec(passport.hgt)[1] >= 59 && hgtRegex.exec(passport.hgt)[1] <= 76) {
                hgt = true
            }
        }

        const hclRegex = /^#\w{6}$/
        if(passport.hcl && hclRegex.test(passport.hcl)) {
            hcl = true
        }
        ...
        const pidRegex = /^\d{9}$/
        if(passport.pid && pidRegex.test(passport.pid)) {
            pid = true
        }
